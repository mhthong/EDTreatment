  ...
  $.fn.repeater = function (fig) {
    ...
    $(this).each(function () {
      ...
      $filterNested($self.find('[data-repeater-create]'), fig.repeaters).click(function () {
        addItem();
        updateIdsForElementsWithMyClass();
      });
...
function updateIdsForElementsWithMyClass() {
  var elementsWithMyClass = document.querySelectorAll('.data-repeater-form');
  var numElementsWithMyClass = elementsWithMyClass.length;
  if (numElementsWithMyClass <= 1) {
    return;
  }
  elementsWithMyClass.forEach(function (element, i) {
    var anchors = element.querySelectorAll(".data-repeater-form a");
    var holder = element.querySelectorAll(".data-repeater-form .holder");
    var inputimage = element.querySelectorAll(".data-repeater-form .data-image-input");
    holder.forEach(function (holder, index) {
      holder.setAttribute("id", "anchor-data-preview-".concat(i, "-").concat(index));
    });
    inputimage.forEach(function (inputimage, index) {
      inputimage.setAttribute("id", "anchor-data-input-".concat(i, "-").concat(index));
    });
    anchors.forEach(function (anchor, index) {
      anchor.setAttribute("id", "anchor-".concat(i, "-").concat(index));
      $('#anchor-' + i + '-' + index).attr('data-preview', "anchor-data-preview-".concat(i, "-").concat(index));
      $('#anchor-' + i + '-' + index).attr('data-input', "anchor-data-input-".concat(i, "-").concat(index));
      // Gọi filemanager cho phần tử mới
      var route_prefix = "/admin/laravel-filemanager";
      $('#anchor-' + i + '-' + index).filemanager('image', {
        prefix: route_prefix
      });
    });
  });
